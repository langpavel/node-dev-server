...
Watcher.prototype._watchDir = function(root) {
  ...
  fs.readdir(root, function(err, files) {
    ...
    files.forEach(function(name) {
      if (Watcher.ignoreNames.indexOf(name) !== -1) return;
...
Watcher.prototype._defer = function(method, relative, stat, oldStat) {
  ...
  data.timeout = setTimeout(this._deferTimeout.bind(this), Watcher.DEFER_TIMEOUT);
};
...
Watcher.prototype._updateStat = function(absolute, relative, err, stat) {
  ...
  if (newStat && !oldStat) {
    ...
    if (newStat.isDirectory()) this._watchDir(absolute);
    ...
    if (newStat.mtime.getTime() !== oldStat.mtime.getTime() || newStat.size !== oldStat.size) {
      this._defer('updated', relative, newStat, oldStat);
    }
  }
};

Watcher.prototype._change = function(root, filename) {
...
module.exports = function(app) {
  app.watch = function() {
    if (!app._watcher) app._watcher = new Watcher(app.root);
